package InterviewProblems.DynamicProgramming.MinCostClimbingStairs.SpaceEfficient;
...
public class MinCostClimbingStairs {
    ...
    // Space: O(1)
    // --> We only store two variables in memory.
    ...
    public int minCostClimbingStairs(int[] cost) {
        int startAt0Cost = cost[0];
        int startAt1Cost = cost[1];
        for (int i = 2; i < cost.length; i++) {
            int minCost = Math.min(startAt0Cost, startAt1Cost) + cost[i];
            startAt0Cost = startAt1Cost;
            startAt1Cost = minCost;
        }
        return Math.min(startAt0Cost, startAt1Cost);
    }
}
