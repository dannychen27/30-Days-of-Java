package InterviewProblems.TwoPointers.FindIntersectionOfTwoLinkedLists.MoreSpaceEfficient;
...
public class IntersectionOfTwoLinkedLists {
    ...
    // Space: O(1)
    // --> We must store the lengths of the linked lists, and pointers to the current positions of the
    // two linked lists.
    ...
    public Node<Integer> getIntersectionNode(Node<Integer> headA, Node<Integer> headB) {
        // Fet length of listA and listB
        int headALength = getLength(headA);
        int headBLength = getLength(headB);

        // Get the difference of the lengths of listA and listB,
        // and move the longer list to the same length of the shorter list
        int diff = Math.abs(headALength - headBLength);
        Node<Integer> longerList = headALength > headBLength ? headA : headB;
        Node<Integer> shorterList = headALength > headBLength ? headB : headA;
        while (diff > 0) {
            longerList = longerList.next;
            diff--;
        }

        // Move the two pointers together until they meet
