package InterviewProblems.Hashmaps.DivideArrayIntoEqualPairs;
...
public class CanDivideArray {
    ...
    // Space: O(n)
    // --> We create a hashmap with the frequencies of each number.
    ...
    public boolean divideArray(int[] nums) {
        int targetNumPairs = nums.length / 2;
        Map<Integer, Integer> numsToFrequencies = recordNumsToFrequencies(nums);
        return isAllPairs(numsToFrequencies, targetNumPairs);
    }

    /**
     * Record the number of occurrences of each number in the input array.
     */
    private Map<Integer, Integer> recordNumsToFrequencies(int[] nums) {
     ...
     */
    private boolean isAllPairs(Map<Integer, Integer> numsToFrequencies, int targetNumPairs) {
        int numPairs = 0;
        for (int num : numsToFrequencies.keySet()) {
            if (numsToFrequencies.get(num) % 2 == 0) {
                numPairs += numsToFrequencies.get(num) / 2;
            }
        }
        return numPairs == targetNumPairs;
    }
}
