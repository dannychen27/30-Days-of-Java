...
public class CanPlaceFlowerSpaceEfficient {

    // Let n = the length of the flowerbed.
    // Time: O(n)
    // --> We must traverse through the flowerbed once.
    // Space: O(1)
    // --> We don't use any extra data structures.

    public static void main(String[] args) {
        CanPlaceFlowerSpaceEfficient canPlaceFlower = new CanPlaceFlowerSpaceEfficient();
    ...
    public boolean canPlaceFlowers(int[] flowerbed, int numFlowersToPlace) {
        for (int i = 0; i < flowerbed.length; i++) {
            // Check if the current plot is nonempty.
            if (flowerbed[i] == 1) {
                continue;
            }
