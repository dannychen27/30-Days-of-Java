package InterviewProblems.Strings.ImplementStrStrAlternativeSolution;
...
public class ImplementStrStr {
    ...
    public int strStr(String haystack, String needle) {
        ...
        for (int i = 0; i < maxStartIndex; i++) {
            if (getSubstring(haystack, i, needle.length()).equals(needle)) {
                return i;
            }
        }
        return -1;
    ...
    private String getSubstring(String word, int startIndex, int length) {
        StringBuilder substring = new StringBuilder();
        for (int i = 0; i < length; i++) {
            substring.append(word.charAt(startIndex + i));
        }
