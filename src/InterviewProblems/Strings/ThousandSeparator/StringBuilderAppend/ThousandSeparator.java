package InterviewProblems.Strings.ThousandSeparator.StringBuilderAppend;
...
public class ThousandSeparator {
    ...
    // --> The while loop body runs in constant time because we're appending characters to a string
    // using a string builder.
    // --> Right before we return from the thousandSeparator method, it takes O(n) time to
    // reverse the entire thousand-separated digit string.
    // Space: O(1)
    ...
    public String thousandSeparator(int n) {
        ...
        while (n >= 1) {
            ...
            thousandSeparatedNumber.append(currentDigit);
            ...
            if ((numDigitsSeen + 1) % 3 == 0 && n >= 10) {
                thousandSeparatedNumber.append(".");
            }

            numDigitsSeen++;
            n /= 10;
        }
        return thousandSeparatedNumber.reverse().toString();
    }
}
